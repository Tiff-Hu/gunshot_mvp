...
class TorchGunshotConfirm:
    ...
    def fit(self, positive_clips: List[np.ndarray], negative_clips: List[np.ndarray], sr: int) -> None:
        ...
        # simple balancing by downsampling larger class
        n = min(len(X_pos), len(X_neg))
        pos_idx = np.random.choice(len(X_pos), size=n, replace=False)
        neg_idx = np.random.choice(len(X_neg), size=n, replace=False)
        ...
        ds = TensorDataset(
            ...
            torch.tensor(y_bal, dtype=torch.float32),
        )
    ...
    for path in files:
        ...
        for trig in triggers:
            ...
            for delta in (-0.20, 0.20):
                ...
                clip_shift, _, _ = extract_centered_clip(audio, sr_, t_shift, clip_s=3.0)
...
def bootstrap_negative_clips(files: List[str], sr: int, max_per_file: int = 3) -> List[np.ndarray]:
    ...
    for path in files:
        audio, sr_ = load_audio(path, sr)
        windows = sliding_windows(audio, sr_, win_s=clip_s, hop_s=hop_s)

        count = 0
        for start, end, _ in windows:
            clip = audio[start:end]
            target_len = int(clip_s * sr_)
            if len(clip) < target_len:
        ...
        if count == 0:
            center_s = len(audio) / (2.0 * sr_)
            clip, _, _ = extract_centered_clip(audio, sr_, center_s, clip_s=clip_s)
            negatives.append(clip.astype(np.float32))
...
def collect_audio_files(audio_dir: str) -> List[str]:
    p = Path(audio_dir)
    files = sorted(
        [str(x) for x in p.rglob("*.mp3")] +
        [str(x) for x in p.rglob("*.wav")] +
        [str(x) for x in p.rglob("*.flac")] +
        [str(x) for x in p.rglob("*.m4a")]
    )
    return files
# =============================
# Main app
...
def main() -> None:
    ...
    ap.add_argument("--gunshot_dir", type=str, required=True, help="Folder containing gunshot audio files.")
    ap.add_argument("--negative_dir", type=str, required=True, help="Folder containing non-gunshot audio files.")
    ...
    if not gunshot_files:
        raise FileNotFoundError(f"No audio files found in gunshot_dir: {args.gunshot_dir}")
    if not negative_files:
        raise FileNotFoundError(f"No audio files found in negative_dir: {args.negative_dir}")
    ...
    print(f"Bootstrapped {len(positive_clips)} positive clips and {len(negative_clips)} negative clips")
    ...
    timeline = build_timeline(gunshot_files, args.sr, shaman_i, shaman_ii)
    ...
    if not args.save_timeline_only:
        summary = {
            "n_gunshot_files": len(gunshot_files),
            ...
            "gunshot_files": [os.path.basename(x) for x in gunshot_files],
        }
