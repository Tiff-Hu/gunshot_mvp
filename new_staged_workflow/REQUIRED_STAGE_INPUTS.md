...
This is the current UI/backend field list for the 5-stage audio workflow. It is based on `Folder Guide.docx`, `cnn_tiny.ipynb`, `CNN_experiments.ipynb`, `modeling_human_presence_analysis_Final.ipynb`, and the runnable Python pipeline files in this folder.
...
- `metadata_json`: Optional node/window metadata JSON. In the current FastAPI router this reuses `ground_truth_log` for compatibility, but the UI should name it metadata.
...
## Open Items for Griffen

- Confirm whether another teammate is already building this combined workflow.
- Provide the final TinyCNN weights and any training normalization constants.
- Confirm whether BirdNET should use location/date filtering in production.
- Provide the final human-presence model artifact and its exact inference-time feature order.
